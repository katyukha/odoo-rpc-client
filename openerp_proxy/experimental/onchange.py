...
class ObjectOnchangeUtils(ObjectUtils):
    ...
    def find_onchanges(view_info=None):
    ...
    def process_onchanges(self, data, fields=None, view_info=None, context=None):
        """ Run onchanges
            ...
            :param dict context: context to prcess onchanges within (*optional*)
        ...
        for field in fields:
            if '.' in field:
                ...
                if xfield not in self.columns_info:
                    continue
                sub_fields[self.columns_info[xfield]['relation']].append(sub_field)
    ...
    def process_onchanges_r(self, data, fields=None, view_info=None, keep_fields=None, context=None):
        if view_info is None:
            view_info = self.get_view_info()
        ...
        # parse keep fields
        k_sub_fields, k_m_fields, k_r_fields = self.parse_model_subfields(keep_fields)

        # run onchanges for current model
        res = self.process_onchanges_x(data, fields=m_fields, view_info=view_info, context=context)
        new_data = data.copy()
        new_data.update(res)
        for k_field in k_m_fields:
            new_data[k_field] = data[k_field]   # remove fields that should not be changed


        for field in r_fields: # new_data.items():
                ...
                for command in value:
                    if command[0] == 0:  # data not written to database
                        ...
                        if not codata.get(comodel_field, False):
                            ...
                            del xdata[field]              # remove current field to avoid recursion
                            zdata = codata.copy()         # copy command data
                            zdata[comodel_field] = xdata  # add data to field that point to parent record
                            z_views = view_info['fields'][field]['views']
                            z_view_info = z_views.get('form', z_views.get('tree', {})) or None
